...
# python -m train $ARGS &
wait
BASE_ON=$(python -m get_run_name $ARGS)
RUN_NAMES+=$BASE_ON" "
...
for TRAIN_BACKBONE in ""; do
  ...
  # python -m methods.early_exit $ARGS &
  WAIT_FOR+=$!" "
  BASE_ON_GRID=$(python -m get_run_name $ARGS)
  ...
  for lr in 0.0005; do
    ...
    # python -m methods.early_exit $ARGS &
    RUN_NAMES+=$(python -m get_run_name $ARGS)" "
  done
done
...
OUTPUT_DIR="figures_resnet50_cifar100"
DISPLAY_NAMES="Base Cascading ZTW"

# cost vs plots
python -m visualize.cost_vs_plot --exp_names $EXP_NAMES --exp_ids $XPIDS --display_names $DISPLAY_NAMES --output_dir $OUTPUT_DIR --use_wandb
python -m visualize.cost_vs_plot --mode calibration --exp_names $EXP_NAMES --exp_ids $XPIDS --display_names $DISPLAY_NAMES --output_dir $OUTPUT_DIR --use_wandb
python -m visualize.cost_vs_plot --mode ood_detection --ood_dataset cifar10 --exp_names $EXP_NAMES --exp_ids $XPIDS --display_names $DISPLAY_NAMES --output_dir $OUTPUT_DIR --use_wandb

# tables
python -m visualize.cost_vs_table --exp_names $EXP_NAMES --exp_ids $XPIDS --display_names $DISPLAY_NAMES --output_dir $OUTPUT_DIR --use_wandb

# HI plots
python -m visualize.hindsight_improvability --exp_names $EXP_NAMES --exp_ids $XPIDS --display_names $DISPLAY_NAMES --output_dir $OUTPUT_DIR --use_wandb